...
function categoriaProvincia($con, $categoria, $provincia, $f_inicio, $f_fin){
    $query="SELECT e.*, t.categoria_evento, g.web_grupo, g.info_grupo, f.web_festival, f.info_festival, p.provincia FROM eventos e INNER JOIN tipo_eventos t ON e.id_tipo = t.id_tipo LEFT JOIN grupos g ON e.id_grupo = g.id_grupo LEFT JOIN festivales f ON f.id_festival = e.id_festival  INNER JOIN provincias p ON p.id_provincia = e.id_provincia WHERE e.id_tipo =$categoria AND e.id_provincia = $provincia AND e.fecha_inicio BETWEEN '$f_inicio' AND '$f_fin'";
    $result = mysqli_query($con, $query);
    return $result;
}
...
function categoria($con, $categoria){
    $query="SELECT e.*, t.categoria_evento, g.web_grupo, g.info_grupo, f.web_festival, f.info_festival, p.provincia FROM eventos e INNER JOIN tipo_eventos t ON e.id_tipo = t.id_tipo LEFT JOIN grupos g ON e.id_grupo = g.id_grupo LEFT JOIN festivales f ON f.id_festival = e.id_festival INNER JOIN provincias p ON p.id_provincia = e.id_provincia WHERE e.id_tipo =$categoria";
    $result = mysqli_query($con, $query);
    return $result;
}
//Devuelve información de todos los eventos gratuitos por categoría
function categoriaGratis($con, $categoria){
    $query="SELECT e.*, t.categoria_evento, g.web_grupo, g.info_grupo, f.web_festival, f.info_festival, p.provincia FROM eventos e INNER JOIN tipo_eventos t ON e.id_tipo = t.id_tipo LEFT JOIN grupos g ON e.id_grupo = g.id_grupo LEFT JOIN festivales f ON f.id_festival = e.id_festival INNER JOIN provincias p ON p.id_provincia = e.id_provincia WHERE e.id_tipo IN ($categoria) AND e.precio = 0 ";
    $result = mysqli_query($con, $query);
    return $result;
}

...
";
  ...
  if (isset($_GET['id_festival'])) {
    ...
    $query_festival = "SELECT DISTINCT e.id_festival, p.provincia, f.* FROM eventos e INNER JOIN provincias p ON p.id_provincia = e.id_provincia LEFT JOIN festivales f ON f.id_festival=e.id_festival WHERE e.id_festival IS NOT NULL AND e.fecha_inicio BETWEEN '$fecha' AND '" . date('Y-m-d', strtotime('+4 months')) . "'=?";
